...
4. Detect whether the user or calling workflow requires draft PR/MR state. Then detect the remote host from `git remote get-url origin` (github.com -> gh, gitlab.com -> glab; default GitHub) and create the PR/MR with the matching CLI. The body is concise bullet points only (no `## Summary`, `## Test Plan`, checklists, or other heading sections):
   - **GitHub**: `gh pr create --assignee @me`, and add `--draft` when draft state is required (`@me` resolves to the authenticated user).
   ...
   - **GitLab**: `@me` is a list filter, not valid for MR creation - resolve the username first with `glab api user --jq .username`, then `glab mr create --assignee <username>`, and add `--draft` when draft state is required.
